...
from multidim_screening_plain.insurance.d1_m1_risk_copay.insurance_d1_m1_risk_copay_plots import (
    plot_calibration,
)
from multidim_screening_plain.insurance.d1_m1_risk_copay.insurance_d1_m1_risk_copay_values import (
    cost_non_insur,
...
def b_fun(
    ...
    if theta is not None:
        ...
        return b_fun_all(model, y, gr=gr)
...
def create_initial_contracts(
    ...
    if start_from_first_best:
        ...
        free_y = list(range(N))
    else:  # not insured if copay is 1
...
def proximal_operator(
    ...
    def prox_obj_and_grad(
        ...
        S_vals = S_fun(model, y, theta, gr=gr)
        if not gr:
            obj = -S_vals
            if t is not None:
                dyz = y - z
    ...
    y_init = np.array([0.7]) if t is None else z
...
def adjust_excluded(results: ScreeningResults) -> None:
    ...
    EPS = 0.001
    MAX_COPAY = results.model.y_minmax[0, 1]
    excluded_types = np.where(deduc > MAX_COPAY - EPS, True, False).tolist()
    results.SB_surplus[excluded_types] = results.info_rents[excluded_types] = 0.0
    n_excluded = np.sum(excluded_types)
    results.SB_y[excluded_types, 0] = np.full(n_excluded, MAX_COPAY)

...
def val_BC(
    ...
    if theta is not None:
        ...
        if not gr:
            ...
            grad = np.zeros(1)
            grad[0] = sigma * s * H_fun(d1) * val_expC
        ...
        if not gr:
            ...
            grad[0, :, :] = s * H_fun(d1) * val_expC * sigmas.reshape((-1, 1))
            return val_compC, grad
...
def S_penalties(y: np.ndarray, gr: bool = False) -> Any:
    ...
    if not gr:
        ...
        grad = np.zeros(1)
        grad[0] = (
            2.0 * coeff_qpenalty_S1_0 * y_1_neg + 2.0 * coeff_qpenalty_S1_1 * y_1_above1
        )

...
from multidim_screening_plain.insurance_d2_m1_copay_values import (
    S_penalties,
    check_args,
    cost_non_insur,
...
def plot_results(model: ScreeningModel) -> None:
    ...
    df_all_results = (
        ...
        .rename(
            columns={
                "FB_y_0": "First-best copay",
                "y_0": "Second-best copay",
    ...
    df_first_and_second = pd.DataFrame(
        {
            ...
            "Copay": np.concatenate(
                (
                    df_contracts["First-best copay"],
                    df_contracts["Second-best copay"],
                )
            ),
        }
    )
    ...
    plot_contract_riskavs(
        df_first_and_second,
        "Copay",
        path=model_plotdir + "/copays_riskavs",
    )
